...
export type SourceConfig<T> = { [P in keyof T]: string };

export type Data<T> = { [P in keyof T]: T[P] };

export function getData<T>(
    sourceConfig: SourceConfig<T>
): () => Promise<Data<T>> {
    return () =>
        new Promise(resolve => {
            let unresolved = Object.keys(sourceConfig).length;
            const result: Data<T> = Object.keys(sourceConfig).reduce(
            ...
            Object.keys(sourceConfig).forEach(key =>
                fetchJson(sourceConfig[key]).then(
                    handleResponse(key),
                    handleResponse(key)
                )
            );
